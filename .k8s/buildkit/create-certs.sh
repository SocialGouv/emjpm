...
(
	...
	kubectl create secret generic ${PRODUCT}-daemon-certs --from-file=./daemon >${PRODUCT}-daemon-certs.yaml
	kubectl create secret generic ${PRODUCT}-client-certs --from-file=./client >${PRODUCT}-client-certs.yaml
)
