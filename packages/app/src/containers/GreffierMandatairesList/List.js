...
function GreffierMandatairesListList(props) {
  const {
    orderBy,
    debouncedSearchText,
    setCurrentOffset,
  ...
  if (coords) {
    ...
    Object.assign(variables, {
      ...
      tribunal: greffier.ti_id,
      departementCode: greffier.ti.departement_code,
      departementFilter: departement,
    });
  }
  ...
  return (
    <>
      {data.mandatairesList.map((item) => {
        ...
        const to = `/greffiers/gestionnaires/${gestionnaire.id}`;
        const onItemClick = (e) => {
          if (e.ctrlKey) {
            return;
          }
...
export { GreffierMandatairesListList };
