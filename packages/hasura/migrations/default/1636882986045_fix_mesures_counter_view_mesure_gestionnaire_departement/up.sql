CREATE OR REPLACE VIEW "public"."view_mesure_gestionnaire_departement" AS 
 SELECT concat('service-', ser.id) AS id,
    ...
    ser.mesures_in_progress_cached AS mesures_in_progress,
        CASE
            WHEN (ser.suspend_activity IS FALSE) THEN ((ser.dispo_max - count_service_mesures_awaiting(ser)::int) - count_service_mesures_in_progress(ser)::int)
            ELSE 0
...
UNION
 SELECT concat(u.type, '-', man.id) AS id,
    ...
    man.mesures_en_cours_cached AS mesures_in_progress,
        CASE
            WHEN (man.suspend_activity IS FALSE) THEN ((man.dispo_max - count_mandataire_mesures_en_attente(man)::int) - count_mandataire_mesures_en_cours(man)::int)
            ELSE 0
        END AS remaining_capacity,
