CREATE OR REPLACE VIEW "public"."view_mesure_gestionnaire_departement" AS 
 SELECT concat('service-', ser.id) AS id,
    ...
    ser.mesures_last_update AS mesures_last_update
   FROM services ser,
    departements dep
  WHERE ((dep.id)::text = (ser.departement_code)::text)
UNION
 SELECT concat(u.type, '-', man.id) AS id,
    ...
    man.mesures_last_update AS mesures_last_update
   FROM mandataires man,
    users u,
    lb_users lbu,
    lb_departements lbd,
    departements dep
  WHERE (((u.type)::text = 'individuel'::text) AND (man.user_id = u.id) AND (man.lb_user_id = lbu.id) AND (man.lb_user_id = lbd.lb_user_id) AND ((lbd.departement_code)::text = (dep.id)::text))
UNION
 SELECT concat(u.type, '-', man.id) AS id,
    NULL::integer AS service_id,
    man.id AS mandataire_id,
   ...
   FROM mandataires man,
    ...
    etablissements etbl,
    departements dep
  WHERE (((u.type)::text = 'prepose'::text) AND (man.user_id = u.id) AND (man.lb_user_id = lbu.id) AND (man.lb_user_id = lbue.lb_user_id) AND (lbue.etablissement_id = etbl.id) AND ((etbl.departement_code)::text = (dep.id)::text));
