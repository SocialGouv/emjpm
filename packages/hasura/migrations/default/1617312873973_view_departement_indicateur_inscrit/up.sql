CREATE OR REPLACE VIEW "public"."view_departement_indicateur_inscrit" AS 
 SELECT d.id,
   ...
   FROM mandataires m,
    users u,
    departements d
  WHERE (((m.departement_code)::text = (d.id)::text) AND (m.user_id = u.id))
...
UNION
 SELECT d.id,
   ...
   FROM users u,
    ...
    service_members sm,
    services s
  WHERE ((u.active = true) AND (u.id = sm.user_id) AND (sm.service_id = s.id) AND ((s.departement_code)::text = (d.id)::text))
  GROUP BY d.id, d.nom, u.type
UNION
 SELECT d.id,
    d.nom,
...
UNION
 SELECT d.id,
  ...
  WHERE ((di.type)::text = 'departemental'::text)
  GROUP BY d.id, d.nom, u.type
UNION
 SELECT d.id,
    d.nom,
   ...
   FROM (((direction di
     ...
     JOIN users u ON ((di.user_id = u.id)))
  WHERE ((di.type)::text = 'regional'::text)
  GROUP BY d.id, d.nom, u.type;
