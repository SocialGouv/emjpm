...
UNION
 SELECT concat('individuel-', man.id) AS uid,
    NULL::integer AS ti_id,
    man.liste_blanche_id AS liste_blanche_id,
    NULL::integer AS service_id,
    man.id AS mandataire_id,
    'individuel'::text AS user_type,
    concat(u.nom, ' ', u.prenom) AS nom,
    man.siret::text,
    man.code_postal::text,
    man.ville::text,
    concat_ws(' '::text, man.adresse, man.adresse_complement) AS adress,
    u.email::text,
    dep.id AS departement_code,
        CASE
   ...
   FROM (((((mandataires man
     ...
     LEFT JOIN geolocalisation_code_postal gcp ON (((gcp.code_postal)::text = (man.code_postal::text)::text)))
