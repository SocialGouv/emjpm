CREATE OR REPLACE VIEW "public"."view_departement_indicateur_inscrit" AS 
 SELECT d.id,
    d.nom,
    u.type,
    count(DISTINCT m.id) AS count
   FROM ((((mandataires m
     JOIN users u ON ((((u.type)::text = 'individuel'::text) AND (m.user_id = u.id))))
...
UNION
 SELECT d.id,
    ...
    count(DISTINCT m.id) AS count
   FROM (((((mandataires m
     JOIN users u ON ((((u.type)::text = 'prepose'::text) AND (m.user_id = u.id))))
     JOIN liste_blanche lb ON ((m.liste_blanche_id = lb.id)))
     JOIN mandataire_prepose_etablissements lbe ON ((m.liste_blanche_id = lbe.liste_blanche_id)))
     JOIN etablissements etbl ON ((lbe.etablissement_id = etbl.id)))
     JOIN departements d ON (((etbl.departement_code)::text = (d.id)::text)))
  WHERE (u.active = true)
  GROUP BY d.id, u.type
UNION
 SELECT d.id,
...
UNION
 SELECT d.id,
  ...
  WHERE (((di.type)::text = 'departemental'::text) AND (u.active = true))
  GROUP BY d.id, d.nom, u.type
UNION
 SELECT d.id,
    d.nom,
   ...
   FROM (((direction di
     ...
     JOIN users u ON ((di.user_id = u.id)))
  WHERE (((di.type)::text = 'regional'::text) AND (u.active = true))
  GROUP BY d.id, d.nom, u.type
UNION
 SELECT
  d.id,
    ...
    (
      (
        (
          ...
          JOIN users u ON (
            (
              ((u.type) :: text = 'dpfi' :: text)
              AND (fi.user_id = u.id)
            )
          )
        )
        JOIN liste_blanche lb ON ((fi.liste_blanche_id = lb.id))
      )
      JOIN dpfi_departements lbd ON ((fi.liste_blanche_id = lbd.liste_blanche_id))
    )
    JOIN departements d ON (((lbd.departement_code) :: text = (d.id) :: text))
  )
 WHERE
 ...
 FROM
  (
      ...
      (
        (
          users u
          JOIN sdpf_members sm ON ((u.id = sm.user_id))
        )
        JOIN sdpf s ON ((sm.sdpf_id = s.id))
      )

    JOIN departements d ON ((s.departement = (d.id) :: text))
  )
 WHERE
  (u.active = true)
 GROUP BY
