...
WHERE id IN (
  ...
  INNER JOIN (
    SELECT min(id) AS min_id, service_id, numero_rg, ti_id, code_postal, annee_naissance, date_nomination, numero_dossier, lieu_vie, nature_mesure, champ_mesure, status, date_protection_en_cours
    FROM mesures
    WHERE service_id IS NOT NULL AND numero_dossier IS NOT NULL AND numero_rg != ''
    GROUP BY service_id, numero_rg, ti_id, code_postal, annee_naissance, date_nomination, numero_dossier, lieu_vie, nature_mesure, champ_mesure, status, date_protection_en_cours
    HAVING   COUNT(*) > 1
  ) m2 ON (
    m.service_id = m2.service_id AND m2.service_id IS NOT NULL AND
