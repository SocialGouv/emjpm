...
UNION
 SELECT
  ...
  d.nom,
  u.type,
  count(DISTINCT sdm.sdpf_id) AS count
FROM
  (
      ...
      (
        (
          users u
          JOIN sdpf_members sdm ON ((u.id = sdm.user_id))
        )
        JOIN sdpf sd ON ((sdm.sdpf_id = sd.id))
      )


    JOIN departements d ON ((sd.departement = (d.id) :: text))
  )
WHERE
  (
    (u.active = true)
