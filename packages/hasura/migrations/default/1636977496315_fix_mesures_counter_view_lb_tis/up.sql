CREATE OR REPLACE VIEW "public"."view_lb_tis" AS 
 SELECT concat('service-', ser.id) AS uid,
    ...
    count_service_mesures_in_progress(ser.*)::int AS mesures_in_progress,
        CASE
            WHEN (ser.suspend_activity IS NOT TRUE) THEN ((ser.dispo_max - count_service_mesures_awaiting(ser.*)::int) - count_service_mesures_in_progress(ser.*)::int)
            ELSE 0
...
UNION
 SELECT concat('individuel-', man.id) AS uid,
    ...
    count_mandataire_mesures_en_cours(man.*)::int AS mesures_in_progress,
        CASE
            WHEN (man.suspend_activity IS NOT TRUE) THEN ((man.dispo_max - count_mandataire_mesures_en_attente(man.*)::int) - count_mandataire_mesures_en_cours(man.*)::int)
            ELSE 0
        END AS remaining_capacity,
    false AS prefer,
        CASE
