...
create view view_indicateur_inscrit as select d.code, d.nom, u.type, count(m.id) from mandataires m, users u, departements d
    where m.department_id = d.id and m.user_id = u.id and u.active = true
    group by d.code, d.nom, u.type
union
select d.code, d.nom, u.type, count(distinct(sm.service_id)) from users u, departements d, service_members sm, services s
...
create view view_indicateur_login as select d.code, d.nom, u.type, count(m.id) from mandataires m, users u, departements d
